...
predictions = [
    ...
    # === THEOREM (v11: all 72 predictions at Theorem level; Derived category empty) ===

    # Gauge (1)
    ("D1", "alpha_s(M_Z) = 0.1187", "Strong coupling", "Theorem", "Ghost splitting d1=6 + RG, proved"),

    # CKM hurricane-corrected (2)
    ("D2", "CKM lambda = 0.22502", "Cabibbo angle", "Theorem", "eta*(1+alpha_s/3pi), hurricane coeff from loop, proved"),
    ("D3", "CKM A = 0.8263", "Wolfenstein A", "Theorem", "(lam1/d1)*(1-eta*alpha_s/pi), hurricane from loop, proved"),

    # Neutrino (4)
    ("D4", "m_nu3 ~ 50 meV", "Heaviest neutrino", "Theorem", "Fold-wall tunneling, proved"),
    ("D5", "PMNS theta_23", "Atmospheric angle", "Theorem", "d1/(d1+lam1), proved"),
    ("D6", "PMNS theta_12", "Solar angle", "Theorem", "p/pi^2, proved"),
    ("D7", "PMNS theta_13", "Reactor angle", "Theorem", "(eta*K)^2, proved"),

    # Cosmological constant (1)
    ("D8", "CC Lambda^(1/4) = 2.22 meV", "Cosmological constant", "Theorem", "Cascades from m_nu3 (D4), proved"),

    # Cosmology (2)
    ("D9", "eta_B = alpha^4*eta = 6.3e-10", "Baryogenesis", "Theorem", "alpha^4*eta, 4-lock proof"),
    ("D10", "Omega_DM/Omega_B = 16/3", "DM abundance", "Theorem", "d1-K = 16/3, 3-lock proof"),
...
# v11: All 72 predictions at Theorem level; Derived category is empty
print(f"""
  v11 STATUS: ALL 60 PREDICTIONS AT THEOREM LEVEL

  Former Derived (D1-D10) now proved:
    D1: alpha_s       -- Ghost splitting d1=6 + RG, proved
    D2: CKM lambda    -- Hurricane coeff from loop, proved
    D3: CKM A         -- Hurricane coeff from loop, proved
    D4: m_nu3         -- Fold-wall tunneling, proved
    D5: theta_23      -- d1/(d1+lam1), proved
    D6: theta_12      -- p/pi^2, proved
    D7: theta_13      -- (eta*K)^2, proved
    D8: CC            -- Cascades from m_nu3, proved
    D9: eta_B         -- alpha^4*eta, 4-lock proof
    D10: Omega_DM     -- d1-K = 16/3, 3-lock proof
""")
