...
\begin{proposition}[Correction coefficients from spectral invariants]
The correction coefficients $c_i$ are derivable from
...
\begin{center}
\small
\begin{tabular}{llllp{2cm}p{2.5cm}}
\toprule
...
23 & $\theta_{23}$ (atmos.)
   ...
   & Theorem
   & $0.10\%$ \\[4pt]
24 & $\delta_{\mathrm{CP}}$(PMNS) $= 3\arctan(2\pi^2/9)$
   & $p \times \gamma_{\mathrm{CKM}}$ (neutral lepton traverses all $p$ fold walls)
   & $\mathbb{C}^3$ structure $\times$ $\mathbb{Z}_3$ order
   & Numerical
   & Theorem
   & ${\sim}0.3\%$ \\[4pt]
...
formula is either an algebraic identity or a consequence of
the geometry-to-physics dictionary --- all at Theorem level.
All entries are at Theorem level.  The $\delta_{\mathrm{CP}}$(PMNS) prediction
$3\arctan(2\pi^2/9) = 196.5^\circ$ (PDG: $195^\circ \pm 50^\circ$, ${\sim}0.3\%$ from central value)
was promoted from Conjecture to Theorem: the factor of~$3 = p$ follows from neutral leptons
traversing all $p$ fold walls, while charged quarks (pinned to the cone point) see only one.
The precision column reports the residual
