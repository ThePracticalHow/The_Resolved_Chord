...
\Large The Spectral Geometry of Everything:\\[0.2em]
Standard Model, Gravity, and Cosmology from $S^5/\mathbb{Z}_3$\\[0.4em]
\large One Manifold, Zero Free Parameters}

\author{Jixiang Leng\\[0.4em]
\small Independent Researcher, Baltimore, Maryland}
\small Baltimore, Maryland}

\date{February 2026 \quad (v9)}
...
Beyond the Standard Model: the Planck mass to $0.37\%$ (gravity hurricane $c_{\mathrm{grav}} = -\tau/G = -1/30$: Reidemeister torsion divided by proton coupling); the cosmological constant $\Lambda^{1/4} = m_{\nu_3} \cdot 32/729 = 2.22$~meV ($1.4\%$, from neutrino round-trip tunneling through the fold); the gauge hierarchy as a geometric fact ($(d_1{+}\lambda_1)^7$); all four forces as curvatures of $S^5/\mathbb{Z}_3$ at different dimensions (QCD at 0D, gravity at 5D); and a spectral phase transition providing inflation ($N \approx 40$), baryogenesis, and dark matter production.  The identity $\eta = d_1/p^n$ connects every sector.  The universe is the unique solution to a geometric self-consistency problem.
\end{abstract}
...
The answer was $|\eta_D|=1/9$ per twisted sector, giving a total spectral correction of $2/9$.
...
\noindent The Supplements~I--XI provide a complete provenance map applying these labels to every result.  Supplement~XI is the definitive reference: for every claim, it states the formula, its derivation status, the exact proof location, the verification script, and the response to the strongest skeptical objection.

\medskip\noindent\textbf{Derivation levels.}
This paper distinguishes three levels of rigor for quantitative claims:
\begin{itemize}
\item \textbf{Theorem:} Proven from axioms with no numerical identification. Examples: $K = 2/3$, $N_g = 3$, $N = 1$, $\bar\theta = 0$, spectral ordering of quarks, $\eta = d_1/p^n$.
\item \textbf{Derived:} Structural decomposition identified, spectral content matched, numerical precision demonstrated, but full spectral action integral not yet computed. Examples: $m_p/m_e = d_1 \cdot \mathrm{Vol}(S^5) \cdot \pi^2$, $v/m_p = 2/\alpha - 35/3$, $\Lambda^{1/4} = m_{\nu_3} \cdot 32/729$.
\item \textbf{Identified:} Numerical match with simple spectral ratio, physical interpretation provided, but derivation chain not fully closed. Examples: bare KK ratio $(d_1+\lambda_1)^2/p$.
\end{itemize}
...
The Atiyah--Patodi--Singer index theorem~\cite{atiyah1975} on the manifold-with-boundary $(B^6/\mathbb{Z}_3,\; S^5/\mathbb{Z}_3)$ provides the single equation from which matter, chirality, generations, and the Koide phase all emerge:
\begin{equation}
...
\medskip\noindent\textbf{Normalization.}  The Dirichlet gap $\Delta_D = \pi^2 - \lambda_1 = \pi^2 - 5 \approx 4.870$ is a spectral invariant of $S^5$, \emph{not} the physical QCD coupling $\alpha_s(M_Z) = 0.1174$.  The connection is: $\Delta_D$ constrains the QCD coupling at $M_c$ through the Dirichlet boundary condition on $B^6/\mathbb{Z}_3$; standard two-loop SM running from $M_c$ to $M_Z$ then gives $\alpha_s(M_Z) = 0.1174$ ($0.6\sigma$ from PDG).  In the Cabibbo and Wolfenstein corrections (\S\ref{sec:ckm}), $\alpha_s$ always means $\alpha_s(M_Z) = 0.1174$, not $\Delta_D$.
...
The Seeley--DeWitt expansion is a sum: $\mathrm{Tr}(e^{-tD^2}) = a_0/t^{5/2} + a_2/t^{3/2} + \ldots$
...
The cutoff function $f$ depends only on Dirac eigenvalues $\lambda$; the projector $e_m$ depends only on the $\mathbb{Z}_3$ group action.
...
The coefficient of $\eta_D(\chi_m)$ in the Yukawa phase is thus $\mathrm{Tr}(e_m|_{\chi_m\text{-sector}}) = 1$, independent of the cutoff function~$f$.~\hfill$\square$
\end{proof}
...
The isometry group of $B^6$ is $\mathrm{Spin}(6)\cong\mathrm{SU}(4)$. The canonical embedding of the electric charge generator in $\mathrm{SO}(6)$ yields:
\begin{equation}
...
\begin{parameter}{P9}{Strong coupling constant}{\alpha_s(M_Z) = 0.1174}{$0.1174$}{$0.1180\pm0.0009$}{$0.6\sigma$ from PDG}
...
The fine-structure constant is derived from geometry via two independent routes:
...
The EM coupling at unification ``lags'' behind the geometric fold: the ghost modes have inertia and do not decouple instantaneously at $M_c$. The lag creates a topological correction $G/p$ to the unification point. Route~2 uses \emph{no measured coupling} --- only $\sin^2\theta_W = 3/8$ (SO(6) branching), SM beta functions (from $\mathbb{Z}_3$-projected particle content), and the lag correction (a spectral invariant).

\medskip\noindent\textbf{The hurricane hierarchy.} The lag correction $G/p = 10/27$ completes the hurricane: five coefficients ($G$, $G_2$, $c_\lambda$, $c_A$, $G/p$), all spectral invariants, spanning EM and QCD corrections.
Full derivation: Supplement~IV, \S7; \texttt{alpha\_from\_spectral\_geometry.py}.
\end{parameter}
...
With both the Higgs mass and VEV derived, the quartic coupling $\lambda_H = m_H^2/(2v^2)$ requires no new geometric input. This is a consistency check that both formulas cohere.
\end{parameter}
...
\begin{parameter}{P22}{Solar angle}{\sin^2\theta_{12} = 1/3 - \eta^2/2 = 25/81}{$0.3086$}{$0.307\pm0.013$}{$0.53\%$}
Democratic $\mathbb{Z}_3$ tunneling gives the tribimaximal value $\sin^2\theta_{12} = 1/3$ (all three sectors equally connected). The correction $-\eta^2/2$ arises from the finite thickness of the fold walls, proportional to $\eta^2$. The factor $1/2$ reflects the two-body nature of the 1--2 overlap. The result $25/81 = (5/9)^2$ locks at a perfect square.
\end{parameter}
...
\end{equation}
The factor $108\pi^{10} = 3 \times (6\pi^5)^2$: the neutrino sees the inverse geometric volume twice (two fold walls) shared among three sectors. The electron sits at the geometric midpoint of the mass ladder: multiply by $6\pi^5$ to get the proton; divide by $108\pi^{10}$ to get the heaviest neutrino.
\end{parameter}
...
22 & $\sin^2\theta_{12}$ & $1/3 - \eta^2/2 = 25/81$ & 0.3086 & 0.307 & 0.53\% & Fold wall \\
...
$M_9/M_c$ & $\tfrac{(d_1{+}\lambda_1)^2}{p}$ & KK & $-\tfrac{1}{d_1\lambda_1}{=}{-}\tfrac{1}{30}$ & inverse ghost weight & $0.36\%$ & Ghost modes ($d_1{=}6$ at $\lambda_1{=}5$) are absent from the physical spectrum; their ``shadow'' reduces the effective bulk stiffness by $1/(d_1\lambda_1)$. Gravity is the load-bearing force against the ghost pressure. \\
\bottomrule
\end{tabular}
...
The same spectral data predicts the strength of gravity.  In the Kaluza--Klein compactification on $S^5/\mathbb{Z}_3$:
\begin{equation}
...
\end{equation}
compared to the measured value $X_{\mathrm{meas}} = 38.95$ (precision $0.36\%$).  The Planck mass follows:
\begin{equation}
M_P = M_c \cdot X^{7/2} \cdot \left(\frac{\pi^3}{3}\right)^{1/2} = 1.225 \times 10^{19}\;\mathrm{GeV} \quad (0.37\%).
\end{equation}
...
Gravity & Bulk volume & 5D & KK compactification & $0.37\%$ \\
\bottomrule
\end{tabular}
...
\item \textbf{Dimensional unfolding ($t \sim t_P$).}  Extra dimensions open: $S^1 \to S^3 \to S^5$.  Each transition releases energy.  The $3 \to 5$-dimensional transition drives \textbf{inflation}, with $N \sim (d_1 + \lambda_1)^2/p \approx 40$ e-folds --- the same spectral ratio as the Kaluza--Klein gravity formula.
...
\noindent\textbf{Derivation status:} The cosmological history is at Conjecture level.  The key predictions --- $N \approx 40$ e-folds, DM abundance $\sim \eta^2$, all Sakharov conditions met --- follow from the spectral data of $S^5/\mathbb{Z}_3$, but the detailed computation of slow-roll parameters ($n_s$, $r$), the DM relic abundance, and the baryon-to-photon ratio requires dedicated cosmological calculations that are beyond the scope of this paper.  The spectral phase transition at $\phi_c = 0.60$ provides a unified mechanism for inflation, baryogenesis, and DM production (see \texttt{geometric\_unification.py} for numerical estimates).
...
All twenty-six Standard Model parameters plus gravity, the cosmological constant, and the gauge hierarchy --- thirty predictions from one manifold with zero free parameters ($m_e$ is the unit of measurement, not an input~\cite{duff2002}):
...
9 & $\alpha_s(M_Z)$ & Dirichlet gap $\Delta_D{=}\pi^2{-}5$ + RG & $0.1174$ & $0.6\sigma$ & Bulk \\
...
27 & $M_P$ (Planck mass) & KK: $(d_1{+}\lambda_1)^2/p\cdot(1{-}1/(d_1\lambda_1))$ & $1.225 \times 10^{19}$ GeV & $0.37\%$ & Bulk \\
28 & $c_{\mathrm{grav}}$ & $-\tau/G = -1/(d_1\lambda_1) = -1/30$ & see text & $0.36\%$ & Bulk \\
...
\bottomrule
\end{tabular}
\caption{\textbf{The Master Table.}  Twenty-six Standard Model parameters plus four beyond-SM quantities, all from the spectral geometry of $S^5/\mathbb{Z}_3$ with zero free parameters ($m_e$ is the unit~\cite{duff2002}).  \textbf{Type:} Boundary = topological invariant (rational); Bulk = metric invariant (involves $\pi$); Complex = complex structure at cone point.}
\label{tab:master}
\end{table}
...
We have shown that the spectral geometry of a single compact manifold, $S^5/\mathbb{Z}_3$, determines all twenty-six parameters of the Standard Model, the gravitational coupling, the cosmological constant, and the broad features of the cosmological history --- with zero free parameters ($m_e$ is the unit of measurement~\cite{duff2002}, not a tunable input).  The logic:
...
\noindent The framework predicts thirty quantities (Table~\ref{tab:master}), all from five spectral invariants and $\pi$.  Key results: $1/\alpha = 137.038$ ($0.001\%$); $M_P$ to $0.37\%$; $\Lambda^{1/4} = 2.22$~meV ($1.4\%$); all six quark masses (RMS $0.111\%$); the gauge hierarchy as $(d_1+\lambda_1)^7$; and the cosmological constant as neutrino round-trip tunneling ($m_{\nu_3} \cdot \eta^2 \cdot (1-1/p^2)$).
