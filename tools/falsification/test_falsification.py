...
class TestStructuralFalsification:
    ...
    def test_three_generations(self):
        """Generation count = p = orbifold order.  Fourth charged lepton falsifies."""
        # The framework derives n_generations from the Z_p orbifold order
        from lotus.core.geometry import S5Z3
        M = S5Z3()
        n_generations = M.p  # computed, not hardcoded
        assert n_generations == 3, f"Expected 3 generations from Z_3, got {n_generations}"

    def test_no_free_quarks(self):
        """l=1 ghost killing forbids free colour-triplet states."""
        # Ghost spectral weight at l=1 must be d1 = 6 (kills the fundamental 3)
        from lotus.core.geometry import S5Z3
        M = S5Z3()
        assert M.d1 == 6, f"d1 should be 6 for quark confinement, got {M.d1}"
        # Confinement criterion: d1 > lambda1 is not required,
        # but d1 must equal the ghost mode count that kills l=1
        assert M.d1 >= M.p, "Ghost modes insufficient to confine quarks"
