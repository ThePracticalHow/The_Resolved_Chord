...
%% ============================================================
\subsection{The lag correction: $\alpha$ derived to $0.001\%$}
...
\noindent\textbf{Physical interpretation.} The electromagnetic coupling at unification ``lags'' behind the geometric fold: the ghost modes have inertia and their spectral weight $G = 10/9$ is redistributed across $p$ sectors before fully decoupling. The lag is a \emph{topological} correction (from the sector structure), not a perturbative one (from loop integrals). It completes the hurricane hierarchy: five spectral coefficients ($G$, $G_2$, $c_\lambda$, $c_A$, $G/p$) spanning EM and QCD corrections, all derived from $\{d_1, \lambda_1, K, \eta, p\}$.
...
\item $\alpha_s = \pi^2 - 5$: the strong coupling constant at the compactification scale is identified with the Dirichlet gap (Parameter~9 of the main text; $\alpha_s(M_Z) = 0.1174$ after RG running, $0.6\sigma$ from PDG~\cite{pdg2024}).
\end{enumerate}
\end{proof}
