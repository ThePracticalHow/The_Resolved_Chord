...
Status audit: both routes to 1/alpha are now Theorem-level (lag = G/p proved).
...
v11: Lag correction G/p proved as APS boundary term (alpha_lag_proof.py).
...
print(f"""
  ...
  But the CLAIM that the lag correction equals G/p:
    "1/alpha_GUT is shifted by G/p at the matching scale"    [PROVED]

  The individual spectral invariants are all proven.
  The gauge coupling offset = G/p follows from the APS spectral asymmetry (proved).
  ...
  PROVED: The lag correction = G/p follows from the APS boundary term
  (see alpha_lag_proof.py).
""")
...
print(f"""
  ...
  STATUS: THEOREM (0.001%)
  Step 6 (lag correction = G/p) proved as APS boundary contribution.
""")
...
print(f"""
  ...
  STATUS: THEOREM (< 10^-4%)
  G, G2 as spectral loop coefficients (proved).
""")

# ======================================================================
#  G AND G2: SPECTRAL COEFFICIENTS (NOW PROVED)
# ======================================================================
...
print(f"""
  ...
  WHAT IS PROVEN:
    ...
    - The spectral action loop expansion of the proton
      mass produces lam1*eta at one-loop and
      -lam1*(d1+eta) at two-loop.
    - The gauge coupling at M_c receives EXACTLY G/p
      from the ghost mode spectral weight (APS boundary term).
...
print(f"""
  THEOREM: The lag correction IS the APS boundary contribution
  to the gauge coupling at the compactification boundary (proved in alpha_lag_proof.py).
...
print(f"""
  CURRENT STATUS: THEOREM (0.001% via Route 1, <10^-4% via Route 2)
