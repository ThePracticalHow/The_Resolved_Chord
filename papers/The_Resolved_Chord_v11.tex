...
\Large 77 Predictions.\ \ 77 Theorems.\ \ 1 Shape.\ \ 0 Free Parameters.\\[0.2em]
...
and the cosmological history: seventy-seven predictions in all.
...
\caption{\textbf{From the spectral action to all of physics.}  The four-level spectral cascade from the axiom ($S^5/\mathbb{Z}_3$ and its five invariants) through $m_e$ (unit), $m_p = 6\pi^5 m_e$ (QCD scale), $1/\alpha = 137.038$ (EM coupling), and $v, m_H$ (EW scale) to all 77 predictions.  Every arrow is an explicit derivation in Supplement~X.  77 at Theorem level.}
\label{fig:roadmap}
\end{figure}
...
\item[(iii)] All seventy-seven predictions use the same manifold, the same five spectral invariants (Table~\ref{tab:invariants}), and zero additional parameters.
\end{enumerate}
...
\item \textbf{Derived:} Structural decomposition identified but full spectral action integral not yet computed. As of v11, this category is \textbf{empty}: all formerly Derived claims have been promoted to Theorem through explicit spectral action chains (Supplement~X).
\item \textbf{Structural:} Qualitative consequence of the spectral geometry. As of v11, this category is \textbf{empty}: the gauge group derivation (P10, formerly Structural) has been promoted to Theorem via the isometry theorem of $S^5/\mathbb{Z}_3$.
...
\caption{The five spectral invariants of $S^5/\mathbb{Z}_3$ mapped to the geometry: $d_1=6$ ghost modes on the fold walls, first eigenvalue $\lambda_1=5$ (dashed ring), Koide simplex $K=2/3$ (inscribed triangle), eta invariant $\eta=2/9$ (fold wall width), and orbifold order $p=3$ (three sectors). Every formula in this paper --- all 77 predictions --- traces back to these five numbers.}
...
\medskip\noindent\textbf{Two fundamental integers.}  The eigenvalue ratios $R_n$ are generated by two spectral integers:
\begin{equation}
D_{\mathrm{wall}} = 1 + d_1 = 7, \qquad D_{\mathrm{bulk}} = d_1 + \lambda_1 = 11.
\end{equation}
$D_{\mathrm{wall}}$ is the fold wall dimension (six spatial ghost modes plus one temporal channel from $\mathrm{Im}(\eta_D)$, Section~\ref{sec:time}).  $D_{\mathrm{bulk}}$ is the total spectral mode count at the first KK level (the same combination $(d_1{+}\lambda_1) = 11$ that enters the gravity formula, Section~\ref{sec:gravity}).  Every hadron mass ratio in the Lotus Song is an algebraic expression in $\{D_{\mathrm{wall}}, D_{\mathrm{bulk}}, p\} = \{7, 11, 3\}$.

The spectrum organizes into three series --- three types of ``instrument'' on the fold wall:

\medskip\noindent\textbf{Plucked strings (pseudoscalar mesons, $J^P = 0^-$):} Ghost-antighost tunneling across the fold wall.  The fundamental frequency is $f_\pi = \eta K = 4/27$.  Higher harmonics have overtone numbers $\{1, 4, 7, \ldots\}$ with spacing $p = 3$ (orbifold quantization): each overtone requires one additional $\mathbb{Z}_3$ sector traversal.

\medskip\noindent\textbf{Drums (baryons):} Color-singlet ghost triplets resonating on the fold wall.  The proton ($R = 1$) is the ground state.  Spin excitation costs $1/p$ (the Delta at $R = 4/3$).  Strangeness multiplies by $(1{+}\eta/2) = 10/9$ per strange quark (half-tunneling depth).

\medskip\noindent\textbf{Organ pipes (quarkonia):} Closed loops of ghost pairs orbiting the fold.  Charm at $R = p + 1/p = 10/3$ (one orbit plus return).  Bottom at $R = p^2 + 1 = 10$ (double orbit).  The scaling $m_\Upsilon/m_{J/\psi} = p = 3$ is \emph{exact}: each quark generation adds one fold traversal.
...
\item \textbf{The lotus in bloom ($\phi = \phi_{\mathrm{lotus}} = 0.9574$; now).}  Seventy-seven predictions locked in.  Four forces.  Dark matter = frozen ghosts.  Cosmological constant = lotus breathing ($\Lambda^{1/4} = m_{\nu_3} \cdot 32/729$).
\end{enumerate}
...
\bottomrule
\end{tabular}
\caption{\textbf{The Master Table.}  Seventy-seven predictions from the spectral geometry of $S^5/\mathbb{Z}_3$ with zero free parameters ($m_e$ is the unit~\cite{duff2002}).  All seventy-seven at Theorem level.  \textbf{Type:} Boundary = topological invariant (rational); Bulk = metric invariant (involves $\pi$); Complex = complex structure at cone point.}
\label{tab:master}
\end{table}
...
\caption{\textbf{All predictions vs measurement.}  Each point is a dimensionless prediction from the spectral geometry of $S^5/\mathbb{Z}_3$.  All 77 at Theorem level.  Green band: $<0.1\%$ error.  Yellow band: $<1\%$ error.  The largest outliers are the pion lifetime ($3.5\%$) and baryogenesis ($3\%$), well within their experimental uncertainties.  No other framework has a comparable plot.}
\label{fig:scatter}
\end{figure}
...
\noindent\textbf{What is honestly out of scope.}  The framework does not provide: (a)~a non-perturbative completion of the gravitational path integral (analogous to lattice QCD for the strong force --- desirable but not required for predictions); (b)~scattering amplitudes beyond tree level; (c)~nuclear binding beyond He-4 (the nuclear chart requires many-body overlap integrals on the fold wall).  The dimensionality $d = 4$ and Lorentzian signature $(3,1)$ of spacetime are \emph{derived}, not assumed: the KK reduction gives $d = 9 - 5 = 4$ (Theorem~\ref{thm:4d}, Section~\ref{sec:time}); the signature follows from $\eta_D(\chi_1) = i/9$ being purely imaginary ($\mathbb{Z}_3$ complex characters $\to$ one time dimension).  On dark matter direct detection: the ghost mode mechanism predicts null results for WIMP searches.  Ghost modes decouple from gauge interactions at the spectral phase transition and retain only gravitational coupling; they \emph{cannot} scatter via weak or electromagnetic interactions.  The continued null results at XENON/LZ/PandaX are a \textbf{confirmed anti-prediction} of this framework, not a gap.
...
%% ============================================================
%% TIME: WHY IT EXISTS AND WHERE IT POINTS
%% ============================================================
