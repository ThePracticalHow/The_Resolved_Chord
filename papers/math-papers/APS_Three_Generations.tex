...
% Define slashed command if slashed package not available
\DeclareRobustCommand{\slashed}[1]{\ooalign{\hfil$#1$\hfil\cr/\hfil\cr}}
...
\begin{proposition}[Kawasaki decomposition on $B^6/\mathbb{Z}_3$]\label{prop:bulk}
The orbifold index on $B^6/\mathbb{Z}_3$ receives contributions from the smooth bulk
(which vanishes by contractibility) and from the cone-point fixed point at the origin:
\begin{equation}
\mathrm{ind}^{\mathrm{orb}}(\slashed{D}) = \underbrace{\frac{1}{3}\int_{B^6}\hat{A}(R)}_{= 0}
+ \text{(cone-point correction)}.
\end{equation}
\end{proposition}

\begin{proof}
We use the Kawasaki orbifold index formula~\cite{kawasaki1981}.
Since $B^6$ is contractible, all Pontryagin classes vanish, so $\int_{B^6}\hat{A}(R) = 0$.
The bulk contribution from the smooth part of $B^6/\mathbb{Z}_3$ is therefore zero:
\begin{equation}
...
the isomorphism $\mathrm{Spin}(6) \cong \mathrm{SU}(4)$ restricts the spinor
representation $\mathbf{4}$ of $\mathrm{SU}(4)$ to $\mathrm{SU}(3)$ as
$\mathbf{4} = \mathbf{3} \oplus \mathbf{1}$ (the fundamental plus a singlet).
The diagonal $\mathbb{Z}_3 = Z(\mathrm{SU}(3))$ acts on the fundamental
$\mathbf{3}$ as scalar multiplication by $\omega$, so
$\mathrm{tr}_{\mathbf{3}}(\omega) = 3\omega$ and $\mathrm{tr}_{\mathbf{1}}(\omega) = 1$.
Therefore:
\begin{equation}
