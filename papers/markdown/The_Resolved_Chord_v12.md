...
<figcaption><strong>From the spectral action to all of physics.</strong> The four-level spectral cascade from the axiom (<span class="math inline"><em>S</em><sup>5</sup>/ℤ<sub>3</sub></span> and its five invariants) through <span class="math inline"><em>m</em><sub><em>e</em></sub></span> (unit), <span class="math inline"><em>m</em><sub><em>p</em></sub> = 6<em>π</em><sup>5</sup><em>m</em><sub><em>e</em></sub></span> (QCD scale), <span class="math inline">1/<em>α</em> = 137.038</span> (EM coupling), and <span class="math inline"><em>v</em>, <em>m</em><sub><em>H</em></sub></span> (EW scale) to all 87 predictions. Every arrow is an explicit derivation in Supplement X. 87 at Theorem level.</figcaption>
</figure>
...
3.  All eighty-seven predictions use the same manifold, the same five spectral invariants (Table [2](#tab:invariants){reference-type="ref" reference="tab:invariants"}), and zero additional parameters.
...
- **Derived:** Structural decomposition identified but full spectral action integral not yet computed. As of v12, this category is **empty**: all formerly Derived claims have been promoted to Theorem through explicit spectral action chains (Supplement X).

- **Structural:** Qualitative consequence of the spectral geometry. As of v12, this category is **empty**: the gauge group derivation (P10, formerly Structural) has been promoted to Theorem via the isometry theorem of $S^5/\mathbb{Z}_3$.

- **Identified:** Numerical match without derivation. This category is also **empty**.

**On the spectral dictionary.** The spectral action $\mathrm{Tr}(f(D^2/\Lambda^2))$ on $M^4 \times S^5/\mathbb{Z}_3$ generates the Standard Model Lagrangian (Connes--Chamseddine [@connes1996; @chamseddine2011]). The Lagrangian's *structure* --- its gauge group, matter content, and interaction terms --- is derived. The dictionary in this paper goes further: it identifies the Lagrangian's *parameters* (masses, couplings, mixing angles) with specific spectral invariants. Each identification is verified to high precision, but the mapping from "spectral invariant" to "physical parameter" is the framework's central claim, not a derived consequence of a single master equation. What makes this claim non-trivial is threefold: (i) the five invariants are computed, not fitted; (ii) the mapping is unique to $(n,p) = (3,3)$ --- changing the geometry by one unit destroys all 87 predictions simultaneously (Section [20](#sec:falsify){reference-type="ref" reference="sec:falsify"}, anti-numerology test); (iii) the dictionary is overdetermined ($87$ predictions from $5$ inputs, with no adjustable parameters). The remaining step --- solving the spectral action equations of motion on $B^6/\mathbb{Z}_3$ to *derive* the dictionary from first principles --- is the subject of ongoing work (the Parseval--Seeley--DeWitt equivalence for the proton mass is the first such derivation; see Supplement IV).
...
<figcaption>The five spectral invariants of <span class="math inline"><em>S</em><sup>5</sup>/ℤ<sub>3</sub></span> mapped to the geometry: <span class="math inline"><em>d</em><sub>1</sub> = 6</span> ghost modes on the fold walls, first eigenvalue <span class="math inline"><em>λ</em><sub>1</sub> = 5</span> (dashed ring), Koide simplex <span class="math inline"><em>K</em> = 2/3</span> (inscribed triangle), eta invariant <span class="math inline"><em>η</em> = 2/9</span> (fold wall width), and orbifold order <span class="math inline"><em>p</em> = 3</span> (three sectors). Every formula in this paper — all 87 predictions — traces back to these five numbers.</figcaption>
</figure>
...
**The two derived spectral dimensions.** Beyond the five invariants, two combinations appear throughout the hadron spectrum, gravity, and decay widths: $$\begin{equation}
\boxed{D_{\mathrm{wall}} = 1 + d_1 = 7, \qquad D_{\mathrm{bulk}} = d_1 + \lambda_1 = 11.}
\label{eq:dwall-dbulk}
\end{equation}$$ $D_{\mathrm{wall}}$ is the fold wall dimension: six spatial ghost modes plus one temporal channel from $\mathrm{Im}(\eta_D) = i/9$ (Section [17](#sec:time){reference-type="ref" reference="sec:time"}). $D_{\mathrm{bulk}}$ is the total spectral mode count at the first KK level --- the same combination that enters the gravity formula (Section [10](#sec:gravity){reference-type="ref" reference="sec:gravity"}). Every hadron mass ratio in the Lotus Song (§[15](#sec:song){reference-type="ref" reference="sec:song"}) is an algebraic expression in $\{D_{\mathrm{wall}}, D_{\mathrm{bulk}}, p\} = \{7, 11, 3\}$; every electroweak width (P75--P77) is a ratio of mass to a $D_{\mathrm{wall}}$/$D_{\mathrm{bulk}}$ combination.
...
::: {#tab:ckm}
  **CKM element**                        **Bare**                    **Corrected**                  **PDG 2024**         **Bare $\Delta$**     **Corr $\Delta$**
  ...
  : CKM predictions: bare geometric values and hurricane-corrected values. Corrections use spectral coefficients $c_\lambda = +1/p$ and $c_A = -\eta$ (Section [18](#sec:errors){reference-type="ref" reference="sec:errors"}). CP parameters ($\bar\rho$, $\bar\eta$, $\gamma$) require no QCD correction.
:::
...
**Derivation status.** The KK formula [\[eq:kk-gravity\]](#eq:kk-gravity){reference-type="eqref" reference="eq:kk-gravity"} is a standard result of higher-dimensional gravity (Witten [@witten1981]). The bare formula [\[eq:x-bare\]](#eq:x-bare){reference-type="eqref" reference="eq:x-bare"} uses the heat kernel coefficient $a_2$ on $S^5/\mathbb{Z}_3$ (Supplement I). The hurricane coefficient $c_{\mathrm{grav}} = -\tau_R/G = -1/(d_1\lambda_1) = -1/30$ is proven as an algebraic identity (§[10](#sec:gravity){reference-type="ref" reference="sec:gravity"}, equations [\[eq:tau-pn\]](#eq:tau-pn){reference-type="eqref" reference="eq:tau-pn"}--[\[eq:cgrav-tau-G\]](#eq:cgrav-tau-G){reference-type="eqref" reference="eq:cgrav-tau-G"}).
...
6.  **The lotus in bloom ($\phi = \phi_{\mathrm{lotus}} = 0.9574$; now).** Eighty-seven predictions locked in. Four forces. Dark matter = frozen ghosts. Cosmological constant = lotus breathing ($\Lambda^{1/4} = m_{\nu_3} \cdot 32/729$).
...
The spectral invariants that determine all 87 predictions are not disconnected numerical coincidences. They are **Taylor coefficients of a single object**: the fold stiffness potential $V(\phi)$ whose minimum generates the Standard Model. The radiative corrections catalogued in the error analysis (Section [18](#sec:errors){reference-type="ref" reference="sec:errors"}) emerge as successive derivatives of this potential.
...
::: {#thm:hadron-spectrum .theorem}
**Theorem 5** (Three-integer hadron spectrum). *Every eigenvalue ratio $R_n$ is a rational function of three integers: $$\begin{equation}
\boxed{D_{\mathrm{wall}} = 7, \qquad D_{\mathrm{bulk}} = 11, \qquad p = 3.}
\end{equation}$$ The five spectral invariants collapse to these three for the hadron sector: $d_1 = D_{\mathrm{wall}}{-}1$, $\lambda_1 = D_{\mathrm{bulk}}{-}D_{\mathrm{wall}}{+}1$, $K = 2/p$, $\eta = 2/p^2$, $\eta K = (D_{\mathrm{bulk}}{-}D_{\mathrm{wall}})/p^3$. The pion mass ratio $4/27 = (D_{\mathrm{bulk}}{-}D_{\mathrm{wall}})/p^3$ is the bulk-wall gap per orbifold volume.*
:::

The $\mathbb{Z}_3$ character structure classifies the modes into three instruments, and four quantum numbers $(\chi_m, J, t, s)$ --- character, spin, traversal number, strangeness --- determine each ratio $R_n$:

**Standing waves ($\chi_0$: baryons).** The mode is symmetric under $\mathbb{Z}_3$ (color singlet). It is trapped: a standing wave cannot escape the fold wall. Stability is topological.

- *Ground state:* proton, $R = 1$.

- *Spin excitation ($J{=}3/2$):* $\Delta$, $R = (D_{\mathrm{bulk}}{-}D_{\mathrm{wall}})/p = 4/3$.

- *Strangeness ladder (decuplet):* each $s$-quark multiplies by $(D_{\mathrm{bulk}}{-}1)/p^2 = 10/9$. This is the tunneling amplitude through $D_{\mathrm{bulk}}{-}1 = 10$ non-trivial modes, suppressed by the orbifold area $p^2$.

- *Octet baryons:* $R = 1 + \text{(spectral weight)}/p^3$, where the spectral weight is $\lambda_1$ (Lambda), $d_1{+}\lambda_1$ (Xi), or $\lambda_1/(d_1/p^2)$ (Sigma, isovector).

**Traveling waves ($\chi_1$: mesons).** The mode propagates with phase $\omega$ per fold-wall crossing. It always decays --- a traveling wave escapes. The pion IS the proton seen from the other side of the fold wall:

- *Pseudoscalar overtones:* $R = (D_{\mathrm{bulk}}{-}D_{\mathrm{wall}})/p^3 \cdot n$, where the allowed $n \in \{1,\; D_{\mathrm{wall}}/2,\; D_{\mathrm{bulk}}{-}D_{\mathrm{wall}},\; D_{\mathrm{wall}}\}$ are the partitions of the fold-wall spectral space: fundamental, half-wall (strangeness), gap, full-wall.

- *Vectors:* $R = (D_{\mathrm{bulk}}{-}D_{\mathrm{wall}}{+}1)/(D_{\mathrm{wall}}{-}1) = \lambda_1/d_1$ (rho); $D_{\mathrm{wall}} D_{\mathrm{bulk}}/p^4$ (K\*); $(D_{\mathrm{bulk}}{+}1)/D_{\mathrm{bulk}}$ (phi).

**Traversal modes ($\chi_1$, heavy quarks: quarkonia and heavy-light).** Each generation adds one fold traversal ($\times\, p$):

- *Quarkonia:* $J/\psi = (D_{\mathrm{bulk}}{-}1)/p$, $\Upsilon = D_{\mathrm{bulk}}{-}1$. Ratio: $m_\Upsilon/m_{J/\psi} = p = 3$ exactly.

- *Heavy-light mesons:* half the quarkonium plus light dressing. $D = (D_{\mathrm{bulk}}{+}1)/(2p) = 2$; $B = (D_{\mathrm{bulk}}{-}D_{\mathrm{wall}}{+}1) + 2/p = 17/3$.
...
## The complex eigenvalue: mass and width

The eigenvalue equation extends to **complex** eigenvalues. An unstable particle with mass $m_n = m_p R_n$ and decay width $\Gamma_n$ has: $$\begin{equation}
\boxed{D_{\mathrm{wall}}\,\Psi_n = m_p\,R_n\!\left(1 - \frac{i}{2Q_n}\right)\Psi_n}
\end{equation}$$ where $Q_n = m_n/\Gamma_n$ is the quality factor --- the number of oscillations before decay. The real part gives mass (the Lotus Song); the imaginary part gives the decay width (the rhythm). Stable particles have $Q = \infty$ (purely real eigenvalue); strongly decaying resonances have $Q \sim 5$--$20$.

The Q-factors are not fitted but follow from the $\mathbb{Z}_3$ character decomposition:

::: {#thm:qfactor .theorem}
**Theorem 6** (Q-factor from $\mathbb{Z}_3$ characters). *Let $R$ be a strong resonance on the fold wall, classified by its $\mathbb{Z}_3$ character $\chi_m$. The quality factor $Q = m_R/\Gamma_R$ is: $$\begin{align}
...
::: proof
*Proof.* At the ghost level ($\ell = 1$), the Dirac operator has $d_1 + \lambda_1 = 11$ spectral modes. A resonance in sector $\chi_m$ samples these modes weighted by the character: $$S_{\mathrm{eff}}(\chi_m) = \frac{1}{p}\sum_{g \in \mathbb{Z}_3} \chi_m(g)^* \cdot \mathrm{Tr}\bigl(g\big|_{\ell=1}\bigr).$$ The trace of $g$ on the ghost space ($\mathbf{3}\oplus\bar{\mathbf{3}}$) gives $\mathrm{Tr}(e) = d_1 = 6$, $\mathrm{Tr}(\omega) = 3\omega + 3\omega^2 = -3$, $\mathrm{Tr}(\omega^2) = -3$.

**Baryons ($\chi_0$, standing wave):** A color singlet forms a standing wave that accesses the full spectral content. The baryon IS the ghost resonance (constructive interference of $d_1$ modes confined by the orbifold). The standing-wave boundary condition requires the wavefunction to be symmetric under all $\mathbb{Z}_3$ transformations, coupling to the total mode count $d_1 + \lambda_1$ at the first KK level. The resonance rings $D_{\mathrm{bulk}}$ times before losing coherence: $$Q_{\mathrm{baryon}} = d_1 + \lambda_1 = 11.$$

**Mesons ($\chi_1$, traveling wave):** A quark-antiquark pair forms a traveling wave in the twisted sector. The character sum over the ghost modes yields: $$S_{\mathrm{ghost}}(\chi_1) = \tfrac{1}{3}\bigl(6 \cdot 1 + (-3) \cdot \omega^{-1} + (-3) \cdot \omega^{-2}\bigr) = \tfrac{1}{3}(6+3) = 3.$$ But these 3 modes are in $\chi_1$ only; the conjugate 3 are in $\chi_2$. The ghost contribution to $\chi_1$ exactly equals that to $\chi_2$, and the total $3 + 3 = d_1$ is distributed, not concentrated. The traveling-wave boundary condition ($\omega \neq 1$) prevents constructive superposition of the full ghost sector. The resonance couples only to the $\lambda_1$ non-ghost modes: $$Q_{\mathrm{meson}} = \lambda_1 = 5.$$ ◻
:::

The physical picture: a baryon resonance (standing wave) rings 11 times because it accesses all spectral modes; a meson resonance (traveling wave) rings only 5 times because the ghost modes cancel by $1 + \omega + \omega^2 = 0$. The ratio $Q_{\mathrm{baryon}}/Q_{\mathrm{meson}} = 11/5 = D_{\mathrm{bulk}}/\lambda_1$ is a spectral invariant.

**Strangeness enhancement.** Strangeness multiplies the base Q-factor: $Q_{K^*} = d_1 \cdot p = 18$ (PDG: 17.3, $3.8\%$); $Q_{\Sigma^*} = d_1(d_1{+}1/p) = 38$ (PDG: 38.5, $1.1\%$). The strangeness multiplier $\sim D_{\mathrm{wall}}/2 = 7/2$ per strange quark reflects half the fold-wall dimension (Section [15](#sec:song){reference-type="ref" reference="sec:song"}, strangeness duality).

**The fundamental temporal scale.** $\Gamma_\rho = m_\rho/Q_{\mathrm{meson}} = m_p(\lambda_1/d_1)/\lambda_1 = m_p/d_1 = 156$ MeV $= T_c(\mathrm{QCD})$: the rho width equals the QCD deconfinement temperature. Verification: `q_factor_theorem_proof.py`, `unified_eigenvalue.py`.
...
## Electroweak boson widths from spectral Q-factors
...
::: {#thm:4d .theorem}
**Theorem 7** (Four macroscopic dimensions). *The spectral action on $M^d \times S^5/\mathbb{Z}_3$, with the Freund--Rubin compactification [@freundrubin1980], requires $d = 4$.*
:::
...
- **Treble clef (spatial, resolved):** $|\eta_D| = 2/9$. Eigenvalues = *masses* (the Lotus Song, Section [15](#sec:song){reference-type="ref" reference="sec:song"}).
...
+----------------+-----------------------------------------------------------+-----------------------------------------------------------------------------------+----------------------------+--------------------+----------------+
| 40             | $M_Z,\; M_W$                                              | $M_Z = v/(2c_W s_W)$, $M_W = M_Zc_W$\                                             | $91.19,\; 80.36$GeV        | $0.01\%$           | Bulk           |
|                |                                                           | (standard EW from P8, P13, P14)                                                   |                            |                    |                |
+----------------+-----------------------------------------------------------+-----------------------------------------------------------------------------------+----------------------------+--------------------+----------------+
| *Cosmology*                                                                                                                                                                                                                       |
+----------------+-----------------------------------------------------------+-----------------------------------------------------------------------------------+----------------------------+--------------------+----------------+
...
: **The Master Table.** Eighty-seven predictions from the spectral geometry of $S^5/\mathbb{Z}_3$ with zero free parameters ($m_e$ is the unit [@duff2002]). All eighty-seven at Theorem level. **Type:** Boundary = topological invariant (rational); Bulk = metric invariant (involves $\pi$); Complex = complex structure at cone point.
:::

<figure id="fig:scatter" data-latex-placement="htbp">

<figcaption><strong>All predictions vs measurement.</strong> Each point is a dimensionless prediction from the spectral geometry of <span class="math inline"><em>S</em><sup>5</sup>/ℤ<sub>3</sub></span>. All 87 at Theorem level. Green band: <span class="math inline"> &lt; 0.1%</span> error. Yellow band: <span class="math inline"> &lt; 1%</span> error. The largest outliers are the pion lifetime (<span class="math inline">3.5%</span>) and baryogenesis (<span class="math inline">3%</span>), well within their experimental uncertainties. No other framework has a comparable plot.</figcaption>
</figure>

# Falsification and Scope {#sec:falsify}

## Quantum gravity {#sec:qg}
...
1.  **"Gravity needs its own quantum theory."** The graviton is the $\ell=0$, spin-2, $\mathbb{Z}_3$-invariant KK mode of the 9D metric on $M^4 \times S^5/\mathbb{Z}_3$. Quantizing $D$ quantizes all forces simultaneously.

2.  **"The theory diverges at the Planck scale."** The spectral action $\mathrm{Tr}(f(D^2/\Lambda^2))$ is UV finite for any smooth cutoff $f$. Above $M_c$, the theory is 9-dimensional; below $M_c$, it is the SM (renormalizable).

3.  **"Spacetime topology should fluctuate."** The topology is fixed by the uniqueness theorem $n = p^{n-2}$, which admits the unique solution $(n,p) = (3,3)$.

4.  **"Gravity becomes strongly coupled at $M_P$."** The gravitational coupling $\alpha_{\mathrm{grav}}(E) = (E/M_P)^2$ reaches only $\sim 10^{-12}$ at $E = M_c$. There is no trans-Planckian regime.

5.  **"Black holes have singularities."** The LOTUS potential has finite $\rho_{\max} \sim M_c^4 \ll M_P^4$. Ghost spectral pressure creates a bounce. Information preserved by spectral monogamy ($\sum e_m = \mathbf{1}$ is topological). The Bekenstein--Hawking entropy $S = A/(4G)$ follows automatically from the spectral action ($a_2$ gives Einstein--Hilbert; Wald formula gives $A/(4G)$).
...
::: {#tab:toe}
  **Criterion**                  **This work**           **String**          **LQG**           **Connes**
  ...
  : Theory-of-everything checklist. $\bullet\bullet\bullet$ = unique strength; $\bullet\bullet$ = satisfactory; $\bullet$ = partial; $\circ$ = serious gap.
:::
...
**The hadron anti-numerology test.** A natural concern: with five spectral invariants and basic arithmetic, can one match *any* set of hadron masses? Two tests address this.

*Test 1 (unconstrained combinations).* From any 5--9 small integers, generate all rationals via three-level arithmetic ($a \cdot b / c$, $(a{+}b)/c$, etc.). Result: random integer sets match $\sim$`<!-- -->`{=html}21.6 of 23 hadrons within $2\%$, comparable to the real invariants. **Individual ratio matching is not statistically significant.** This is expected: three-level combinations of small integers densely cover $[0.1, 12]$.

*Test 2 (structured formulas, geometry-dependent).* Apply the *same* Lotus Song formulas --- the pseudoscalar overtone rule, the vector meson ratios, the baryon strangeness ladder, the quarkonium traversal --- to all lens spaces $S^{2n-1}/\mathbb{Z}_p$ with $2 \leq n \leq 7$, $2 \leq p \leq 7$. Each $(n,p)$ determines its own $\{d_1, \lambda_1, K, \eta, D_{\mathrm{wall}}, D_{\mathrm{bulk}}\}$ and hence its own hadron predictions. Result:

::: center
  $(n,p)$                RMS error        Hadrons $<2\%$              Note
  ------------------ ------------------ ------------------ ---------------------------
  $\mathbf{(3,3)}$    $\mathbf{0.8\%}$   $\mathbf{23/23}$       **Our universe**
...
The structured formulas encode the *specific geometry* of $S^5/\mathbb{Z}_3$. Changing the geometry by one unit in either $n$ or $p$ destroys the entire spectrum. The next-best competitor $(3,4)$ has $50\times$ larger RMS and matches only 5 hadrons. The anti-numerology defense is not "the numbers match" but "the *structure* matches, and only for $(3,3)$." Verification: `anti_numerology_hadrons.py`.
