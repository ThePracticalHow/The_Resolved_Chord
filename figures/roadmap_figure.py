#!/usr/bin/env python3
"""
THE ROADMAP: From Tr(f(D^2)) to all 87 predictions.
...
ax.text(6, 13.05, r'$\mathrm{Tr}(f(D^2/\Lambda^2))$ on $M^4 \times S^5/\mathbb{Z}_3$'
        r'  $\rightarrow$  87 predictions, zero free parameters',
...
for bx, title, content, arrow_label in positions:
    ...
    rbox(cx_b, branch_y_box, bw_branch, bh, content,
         c_theorem, c_theorem_edge, fontsize=7.5)

# ── Scorecard bar ──
rbox(0.3, 2.4, 11.4, 0.8,
     '87 predictions   |   87 Theorems   |   0 Gaps   |   '
     '87 predictions from 5 invariants + π   |   Zero free parameters',
     c_bar, c_bar, fontsize=10.5, bold=True, text_color='white')

# ── Bottom note ──
ax.text(6, 1.9, 'Every arrow is an explicit derivation in Supplements X and XIV.',
