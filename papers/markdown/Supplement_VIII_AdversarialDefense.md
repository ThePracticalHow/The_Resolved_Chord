...
1.3
Prediction: derivability of correction coefficients
Proposition 1 (Correction coefficients from spectral invariants). The correction coeffi-
cients ci are derivable from the five spectral invariants {d1, λ1, K, η, p} of the Z3 orbifold,
...
is either an algebraic identity or a consequence of the geometry-to-physics dictionary —
all at Theorem level. The single Conjecture entry (δCP(PMNS)) is awaiting experimental
precision. The precision column reports the residual between the bare prediction and the
PDG central value.
11
