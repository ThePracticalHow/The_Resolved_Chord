...
\begin{remark}[Status: Theorem]
The identification $\mathrm{SU}(2)_L = $ ``symmetry of the character block'' follows from the automorphism group $\mathrm{Aut}(\mathbb{Z}_3) = \mathbb{Z}_2$, which embeds into $Z(\mathrm{SU}(2))$ and generates the weak isospin doublet structure via the spectral action on $M^4 \times S^5/\mathbb{Z}_3$.  The $\mathbb{Z}_3$ character decomposition $\{\chi_0, \chi_1, \chi_2\}$ with $\chi_1 \leftrightarrow \chi_2$ under conjugation provides the $\mathrm{SU}(2)$ doublet.  Promoted from Conjecture in v12.
\end{remark}
...
$\mathrm{SU}(2)_L$ from character block & Automorphism structure & $\chi_1, \chi_2$ doublet & Theorem \\
