...
\begin{align}
\eta_D(\chi_1) &= +\frac{1}{9}, \label{eq:eta1}\\
\eta_D(\chi_2) &= -\frac{1}{9}. \label{eq:eta2}
\end{align}
Both values are \textbf{real} (rational).  The imaginary cotangent factors cancel via
$(\omega^2 - \omega) = -i\sqrt{3}$, turning the $i/\sqrt{3}$ prefactor real.
\end{proposition}
...
$2n^{n+3} = (n-1) n^{2n}$, hence:
\begin{equation}
\boxed{2n^3 = (n-1)\,n^n.}
\label{eq:diophantine}
\end{equation}
Verification: for $n = 3$: $2 \cdot 27 = 2 \cdot 27$.\ \ $54 = 54$.\ $\checkmark$
