...
alpha = 1/137.036
phi_lotus = 1 - alpha*(d1 + lam1 + K)/2  # computed from spectral invariants (~0.9574)
M_P = 1.221e19   # GeV, measured Planck mass
M_c = 1.03e13    # GeV, compactification scale (input parameter)
...
print(f"""
  ...
  5. MAXIMUM COMPRESSION
     The ghost spectral pressure (the same force that stabilizes phi_lotus
     in empty space) sets an upper bound on local fold depth.
     Ghost resistance = alpha*(d1+lam1+K)/2 = 1 - phi_lotus.

     phi_max depends on local curvature:
       Planck-mass BH: phi_max ~ 1 - 1/(d1*lam1) = 29/30 ~ 0.967
       Solar-mass BH:  phi_max ~ phi_lotus + O(M_c^2/M_BH^2)
