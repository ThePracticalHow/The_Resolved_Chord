...
Usage:
  python run_falsification.py              # Run full suite
  python run_falsification.py -m "not slow" # Skip slow tests
...
def main():
    ...
    cmd = [sys.executable, "-m", "pytest", "falsification/", "-v" if args.verbose else "-q"]
    if args.markers:
    ...
    try:
        ...
        with open(root / args.results, "w", encoding="utf-8") as f:
